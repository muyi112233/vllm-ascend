# 基于 Yuanrong 的 GLM-5 W8A8 A3 双机 PD 分离部署

## 概述

本指南提供在 2 台 Atlas 800I A3 / Atlas 800T A3 服务器上部署 GLM-5 W8A8 模型、使用 1P1D（1 个 Prefill 节点 + 1 个 Decode 节点）架构，并叠加 Yuanrong Datasystem 作为 KV Pool 后端的参考方案。
...
下载 GLM-5 W8A8 模型权重并放置到指定目录，例如 `/home/models/GLM-5-w8a8/`。

> **模型下载地址**：[魔搭社区](https://modelscope.cn/models/Eco-Tech/GLM-5-w8a8)
...
MODEL_PATH="/home/models/GLM-5-w8a8"
...
| `VLLM_ASCEND_ENABLE_FUSED_MC2` | `1` | A3 上 `GLM-5-w8a8` 可开启融合 MC2，但该能力仍为实验特性 |
...
5. **启用 `VLLM_ASCEND_ENABLE_FUSED_MC2=1` 后启动或首个请求失败**

   这是 A3 上 `W8A8` 的实验融合路径。建议先回退为：
   ```bash
   export VLLM_ASCEND_ENABLE_FUSED_MC2=0
   ```
   如果回退后恢复正常，可继续使用非融合路径排查其余问题。
...
- [基于 Yuanrong 的 GLM-5 W4A8 A3 双机 PD 分离部署](pd_disagg_yuanrong_glm5_a3_dual_node_cn.md)
