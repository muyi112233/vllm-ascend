...
> 本教程默认使用 `GLM-5.1-w8a8` 权重和 A3 镜像 `quay.io/ascend/vllm-ascend:v0.18.0-a3`。如果镜像中已包含 GLM-5.1 和 Yuanrong 所需依赖，可跳过对应安装步骤。
>
> **当前版本补丁要求**：v0.18.0 环境下使用 Yuanrong 多级缓存前，仍需要单独提供并应用以下 patch。建议先将 patch 上传到容器内固定目录 `/workspace/yuanrong_patches/`，然后依次执行：
>
> ```bash
...
> ```
>
> 其中，`0001-Bugfix-Fix-negative-local_cache_hit-in-P-D-disaggreg.patch` 需要打到 `/vllm-workspace/vllm` 仓库下，用于修复 PD 分离场景下 vLLM 日志 / metrics 中 `local_cache_hit` 指标出现负值的问题；`0001-Implement-yuanrong-backend.patch` 需要打到 `/vllm-workspace/vllm-ascend` 仓库下，用于补充 `AscendStoreConnector` 的 Yuanrong backend 支持。若当前环境中已包含对应 patch 改动，可跳过相应步骤。
>
...
| 组件 | 版本建议 | 说明 |
|------|----------|------|
| vLLM Ascend 镜像 | `v0.18.0-a3` | A3 Ubuntu 镜像 |
...
常规部署中，若灵衢组网和 NPU 通信网络已由环境侧完成验收，可跳过该检查。所有关键链路应处于可用状态后再启动推理服务。
...
Yuanrong Worker 使用共享内存时需要预留内存大页。本文按默认 2 MB HugePages 估算，`nr_hugepages` 应不小于 `SHM_SIZE / 2`。在每个宿主机节点启动容器前执行：

```bash
# Prefill 节点（节点 0、节点 1），对应 SHM_SIZE=512000 MB
echo 256000 > /proc/sys/vm/nr_hugepages
grep Huge /proc/meminfo

# Decode 节点（节点 2、节点 3），对应 SHM_SIZE=102400 MB
echo 51200 > /proc/sys/vm/nr_hugepages
grep Huge /proc/meminfo
```

如果系统 `Hugepagesize` 不是 2 MB，请按实际页大小换算。需要持久化时，可将 `vm.nr_hugepages=<大页数量>` 写入 `/etc/sysctl.conf` 或 `/etc/sysctl.d/*.conf`。
...
| 分组 | Docker `--shm-size` | Yuanrong `SHM_SIZE` | HugePages `nr_hugepages` | 说明 |
|------|---------------------|---------------------|---------------------------|------|
| Prefill 节点 | `500g` | `512000` | `256000` | 节点 0、节点 1 |
| Decode 节点 | `100g` | `102400` | `51200` | 节点 2、节点 3 |

`SHM_SIZE` 单位为 MB，会传给 `dscli start -w --shared_memory_size_mb`。Docker `--shm-size` 不应小于 Yuanrong Worker 的共享内存配置。大页数量按默认 2 MB HugePages 估算，如实际页大小不同需同步调整。
...
| `--enable_worker_worker_batch_get` | `true` | Yuanrong Worker 开启 worker 间 batch get |
